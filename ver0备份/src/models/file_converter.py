...
class matFileConverter:
    ...
    PARAM_KEYWORDS = ["index", "param", "ri", "radius", "diam", "height", "width", "period", "gap", "thick"]
    ...
    def _keyword_score(self, name: str, keywords: list[str]) -> int:
        ...
        for word in keywords:
            ...
            if tail == word:
                score += 6
            elif tail.startswith(word):
                score += 5
            elif f"_{word}" in tail or f"-{word}" in tail:
                score += 4
            elif word in tail:
                score += 3
            elif word in low:
    ...
    def get_available_datasets(self, path: str) -> dict[str, list[str]]:
        ...
        try:
            ...
            spec_names = self._rank_candidates(entries, self.SPECTRUM_KEYWORDS, lambda e: bool(e["matrix_like"]))
            if not spec_names:
    ...
    def convert_single_mat_to_csv(
        ...
        param_key = self._pick_key(vector_candidates, param_dataset_name, ["index", "param", "ri"])
