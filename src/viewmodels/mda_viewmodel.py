...
class MdaViewModel(BasePlotViewModel):
    ...
    def __init__(self, i18n: I18NManager, parent: QObject | None = None) -> None:
        ...
        self.last_selected_sweep_values: list[Any] | None = None
    ...
    def _refresh_last_plot_if_needed(self) -> None:
        ...
        if not varying:
            ...
            if not fixed:
                fixed = defaults
        if varying:
            self.plot_sweep(varying, fixed, self.mode, selected_values=self.last_selected_sweep_values)
    ...
    def _collect_metrics_rows(
        ...
        wl, spectra, values_raw = dataset.get_sweep(
            picked_vary,
            picked_fixed,
            selected_values=selected_values,
        )
    ...
    def plot_single(self, selection: dict[str, Any]) -> None:
        ...
        self.last_selected_sweep_values = None
        try:
    ...
    def plot_sweep(
        ...
        self.last_selected_sweep_values = list(selected_values) if selected_values is not None else None

        try:
            wl, spectra, values_raw = self.dataset.get_sweep(
                varying_param,
                fixed_params,
                selected_values=selected_values,
            )
