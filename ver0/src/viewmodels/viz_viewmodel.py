...
from src.models.metrics_calc import build_summary_row, calculate_batch_metrics
...
class VizViewModel(BasePlotViewModel):
    ...
    def _build_line_labels(self, dataset: SpectrumDataset, indices: list[int] | None = None) -> list[str]:
        labels: list[str] = []
        if dataset.spectra_data is None:
            return labels

        picked = list(indices) if indices is not None else list(range(len(dataset.spectra_data)))
    ...
    def _resolved_selected_indices(self, total: int) -> list[int]:
        if total <= 0:
            return []
        ...
        for idx in self.selected_indices:
            i = int(idx)
            if 0 <= i < total and i not in picked:
                picked.append(i)
        return picked
    ...
    def set_selected_indices(self, indices: list[int] | None, *, refresh: bool = True) -> None:
        self.selected_indices = None if indices is None else [int(i) for i in indices]
        if refresh and self.current_mode != "single":
            self.refresh_plot()
    ...
    def refresh_plot(self) -> None:
        ...
        if self.current_mode == "multi":
            picked = self._resolved_selected_indices(len(spectra))
            if not picked:
                self._emit_info("warning", "common.warning", self._t("mda.message.select_sweep_values"))
                return
            spectra_picked = spectra[picked]
            labels = self._build_line_labels(self.dataset, picked)
            ...
            self.plot_data_changed.emit(payload)
            self._update_metrics_summary(picked)
            return
        ...
        if not picked:
            ...
            return
        spectra_picked = spectra[picked]
        params_picked = np.asarray(params, dtype=float)[picked]
    ...
    def _update_metrics_summary(self, selected_indices: list[int] | None = None) -> None:
        if (
            selected_indices is None
            or self.dataset.wavelengths_nm is None
            or self.dataset.spectra_data is None
        ):
            summary = self.dataset.get_batch_summary()
        else:
            spectra = np.asarray(self.dataset.spectra_data, dtype=float)[selected_indices]
            if self.dataset.param_values_scaled is not None:
                params = [self.dataset.param_values_scaled[i] for i in selected_indices]
            else:
                params = [self.dataset.param_values_raw[i] for i in selected_indices]
            batch = calculate_batch_metrics(
                self.dataset.wavelengths_nm,
                spectra,
                param_values=params,
                param_name=self.dataset.param_name,
            )
